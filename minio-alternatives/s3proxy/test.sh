...
until docker compose exec mc mc ls s3proxy >/dev/null 2>&1; do
    echo "...waiting for s3proxy..."
    sleep 2
done
...
echo "1. Checking s3proxy buckets (before)..."
docker compose exec mc mc ls s3proxy
echo ""
...
LOAD iceberg;
LOAD httpfs;
CREATE SECRET s3_secret (TYPE S3, KEY_ID 'admin', SECRET 'password', REGION 'us-east-1', ENDPOINT 's3proxy:8000', USE_SSL false, URL_STYLE 'path');
CREATE SECRET iceberg_secret (TYPE iceberg, TOKEN 'dummy');
ATTACH 'warehouse' AS cat (TYPE iceberg, ENDPOINT 'http://iceberg-rest:8181', SECRET iceberg_secret);
...
echo "4. Checking s3proxy bucket contents (after)..."
docker compose exec mc mc ls --recursive s3proxy/warehouse/test/products/ | head -10
echo ""
